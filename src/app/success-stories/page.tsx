...
import { CalendarDays, Heart, Quote, Star } from 'lucide-react'
import { createClient } from '@/lib/supabase/server'
import { isSupabaseConfigured } from '@/lib/env'
import { photoUrl } from '@/lib/profile/photos'
import { getSuccessStoryCta } from '@/lib/success-stories/routing'
...
export default async function SuccessStoriesPage() {
  ...
  const cta = await getSuccessStoryCta(supabase, user?.id)
      ...
      <div className="container-page py-12 sm:py-16">
        ...
        {stories.length === 0 ? (
          <div className="mx-auto mt-12 max-w-2xl rounded-[26px] border border-stone-200 bg-white/80 px-8 py-12 text-center shadow-card-float">
            ...
            <Link href={cta.href} className="btn-primary mt-6 inline-flex items-center gap-2">
          ...
          <>
                ...
                <li key={s.id} className="flex flex-col overflow-hidden rounded-[26px] bg-white shadow-card-float ring-1 ring-stone-100">
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {s.rating && (
                        <div className="flex items-center gap-0.5 text-amber-500" title={`${s.rating} of 5 stars`}>
                          {Array.from({ length: 5 }).map((_, i) => (
                            <Star
                              key={i}
                              className={`h-3.5 w-3.5 ${i < s.rating! ? 'fill-amber-400 text-amber-500' : 'text-stone-300'}`}
                            />
                          ))}
                        </div>
                      )}
                    ...
                    {s.future_members_note && (
                      <div className="mt-4 border-t border-stone-100 pt-3 text-xs italic text-stone-500">
                        &ldquo;{s.future_members_note}&rdquo;
                      </div>
                    )}
                  </div>
                </li>
            ...
            <div className="mx-auto mt-14 max-w-2xl rounded-[26px] border border-gold-300/60 bg-white/90 px-8 py-10 text-center shadow-card-float">
              <Heart className="mx-auto h-8 w-8 fill-brand-100 text-brand-600" />
              <h2 className="mt-3 font-display text-2xl font-bold text-maroon">
                Did you find your life partner on Mali Vivah?
              </h2>
              <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-stone-600">
                Your journey may inspire another family to take the first step.
              </p>
              <Link href={cta.href} className="btn-primary mt-5 inline-flex items-center gap-2">
                <Heart className="h-4 w-4 fill-current" />
                Begin Story
              </Link>
            </div>
