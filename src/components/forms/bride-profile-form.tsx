...
export function BrideProfileForm({ initial }: { initial?: BrideProfile }) {
  const { t } = useI18n()
  const [form, setForm] = useState<BrideProfile>({
    name: initial?.name || '',
    age: initial?.age || 26,
    city: initial?.city || 'Mumbai',
    subCommunity: initial?.subCommunity || 'Mali',
    education: initial?.education || 'B.E.',
    occupation: initial?.occupation || 'Working Professional',
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setForm(prev => ({ ...prev, [name]: value === '' ? (name === 'age' || name === 'partnerEducation' || name === 'partnerOccupation') ? '' : 0 : parseInt(value) }))
  }
    ...
    <section className="bg-cream">
      <div className="container-page py-24">
        <div className="mx-auto max-w-2xl">
          <h1 className="font-display text-3xl font-bold text-maroon mb-6">{t('pages.brides.title')}</h1>
